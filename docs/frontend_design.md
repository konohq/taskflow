...
このドキュメントは、TaskFlow AI のフロントエンド実装に入る前に、画面構成、UI 方針、API 連携、状態管理、実装順序を整理するためのものです。

バックエンド MVP は実装済みであり、フロントエンドは既存の Rails API を利用する React SPA として実装します。添付画像は UI レイアウトと雰囲気の参考に留め、画像内のタスク名、ユーザー名、日付、会社名、数値、アクティビティ、コメント内容は固定データとして使用しません。
...
| メンバー管理 | email によるメンバー追加、role 変更、削除を行う |
| プロジェクト一覧 | チーム内のプロジェクトを表示する |
| プロジェクト詳細 | プロジェクト概要、KPI、カンバンを表示する中心画面 |
| タスク詳細 / 編集 | タイトル、説明、担当者、ステータス、優先度、期限を編集する |
| コメント | タスクに紐づくコメント一覧と作成フォームを表示する |
| マイタスク | 自分が担当者のタスクを一覧表示する |
...
`/` はダッシュボード画面として扱います。メンバー管理、チーム内プロジェクト一覧、設定などは、MVP では専用ルートを増やさず、チーム詳細画面やプロジェクト詳細画面内の表示として扱います。
...
サイドバーの「プロジェクト」は、MVP では選択中チームのプロジェクト一覧へ誘導します。チーム未選択の場合は、チーム一覧またはチーム選択画面へ誘導します。バックエンドには全チーム横断の `GET /api/v1/projects` はないため、全プロジェクト横断一覧は MVP 後回しにします。
...
| プロジェクト一覧 | GET | `/api/v1/teams/:team_id/projects` |
...
Task API、My Tasks API、Kanban API の `assignee` / `created_by` は `id` と `name` のみに限定されます。`email`、`password`、`encrypted_password`、`jti` は表示しません。
...
| 期限超過 | `due_on` が今日より前かつ `status !== "done"` のタスク数 |
...
画面では、自分が担当者になっているタスクのみを一覧表示します。レスポンスには Project 情報と Team 情報も含まれるため、どのチーム、どのプロジェクトのタスクか分かるように表示します。
...
- owner のみチーム更新、削除導線を表示する

### TeamMember

- メンバー一覧を表示する
- owner または admin のみメンバー追加フォームを表示する
- owner のみ role 変更 UI を表示する
- owner 自身の role 変更と owner への変更は UI でも抑止する
- admin は member 追加のみ可能にする
...
- Project の作成、更新、削除は既存 API を使う

### Task

- タスク一覧とカンバンカードから詳細を開く
- 作成、更新、削除は既存 Task API を使う
...
5. チーム一覧、チーム詳細、メンバー管理の実装
6. プロジェクト一覧、プロジェクト詳細の実装
7. Kanban API を使ったプロジェクト詳細ボードの実装
8. タスク作成、詳細、編集、削除の実装
9. 選択中タスクのコメント一覧、コメント作成の実装
10. My Tasks 画面と絞り込みの実装
11. レスポンシブ対応、空状態、エラー状態、ローディング状態の調整
12. UI の最終確認と不要な mock データの削除
