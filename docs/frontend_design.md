...
- My Tasks の連続検索・リセット・再読み込み時の非同期競合対策
...
| メンバー管理 | 現状はメンバー一覧表示まで実装済み。追加、role 変更、削除 UI は MVP 後回し |
...
- メンバー一覧を表示する
- メンバー追加、role 変更、削除 UI は後回しにする
- 追加、role 変更、削除を実装する場合はバックエンドの owner / admin 制御と同じ条件を UI でも扱う
...
- Task 詳細、Comment、My Tasks の連続取得で古いレスポンスを適用しない
